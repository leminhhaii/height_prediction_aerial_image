...
Context
The current training pipeline has two loss strategies — PixelLoss (MAE + Sobel gradient on latents, prediction_type="sample") and NoiseLoss (MSE on noise, prediction_type="epsilon"). The research document "Enhancing nDSM Prediction with Advanced Losses" identifies two critical failure modes:

Ground-class imbalance: Ground pixels (80-95% of area) dominate gradient descent, producing deceptively low RMSE but visually poor elevated structures.
Blurred height discontinuities: Standard losses don't enforce sharp object boundaries or distinguish which side of an edge is object vs ground.
...
Foundation: gradient_utils.py + update pixel_loss.py import + extend LossConfig + trainer kwargs
Imbalance losses: bmse_loss.py, focal_regression_loss.py, dbr_loss.py
Geometric losses: directional_gradient_loss.py, laplacian_loss.py, surface_normal_loss.py
Orchestration: composite_loss.py + __init__.py imports
Configs: All 5 YAML experiment files
Verification
Backward compat: Run python tools/train.py --config configs/train_pixel_loss.yaml with --total_steps 10 — must work identically
Individual loss test: Run each new loss type standalone: python tools/train.py --config configs/train_bmse.yaml --total_steps 10
Composite test: Run python tools/train.py --config configs/train_composite_curriculum.yaml --total_steps 10 — check that only BMSE fires at step 0
Check logs: Verify per-component loss metrics appear in training logs (e.g., loss_bmse, loss_dir_grad, weight_bmse, etc.)